...
router.post('/send-otp', async (req, res) => {
  ...
  try {
    ...
    const url = `https://2factor.in/API/V1/${TWO_FACTOR_API_KEY}/SMS/${mobileNumber}/AUTOGEN`;
    const response = await axios.get(url);

    if (response.data.Status === 'Success') {
      const sessionId = response.data.Details;
      const token = jwt.sign({ sessionId, mobileNumber, username }, JWT_SECRET, { expiresIn: '10m' });
